...
try:
    ...
    notification.notify(title="C.A.P.T.A.I.N Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("C.A.P.T.A.I.N Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
try:
    import subprocess
    tts_msg = "Boss, check your reminder. Alarm started, check your work. Reminder: " + message.replace("'", "")
    ps_cmd = f"Add-Type -AssemblyName System.Speech; (New-Object System.Speech.Synthesis.SpeechSynthesizer).Speak('{{tts_msg}}')"
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "C.A.P.T.A.I.N Reminder"'.format(
            message.replace('"', '')
        )
...
try:
    import subprocess
    tts_msg = "Boss, check your reminder. Alarm started, check your work. Reminder: " + message.replace('"', '')
    subprocess.run(["say", tts_msg], check=False)
except Exception:
    pass
"""
    ...
    try:
        ...
        subprocess.run(
            ["notify-send", "--urgency=normal", "--expire-time=15000",
             "C.A.P.T.A.I.N Reminder", message],
            check=False
        )
    except Exception:
        pass
...
try:
    import subprocess
    tts_msg = "Boss, check your reminder. Alarm started, check your work. Reminder: " + message.replace('"', '')
    subprocess.run(["spd-say", tts_msg], check=False)
except Exception:
    pass
"""

    script_body = f"""# Auto-generated by C.A.P.T.A.I.N reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>C.A.P.T.A.I.N Reminder</Description></RegistrationInfo>\n'
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.captain.reminder.{task_name}"
...
def reminder(
    ...
    task_name  = f"CAPTAINReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
