...
def brightness_set(value: int):
    ...
    if _OS == "Windows":
        try:
            subprocess.run(
                ["powershell", "-Command", f"(Get-WmiObject -Namespace root/wmi -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, {value})"],
                capture_output=True, timeout=5
            )
        except Exception as e:
            print(f"[Settings] Brightness set failed on Windows: {e}")
    elif _OS == "Linux":
...
def battery_status():
    try:
        ...
        if not battery:
            return "Battery not found or not supported."
...
def system_usage():
    try:
        ...
        return f"Could not get system usage: {e}"
...
def empty_recycle_bin():
    if _OS == "Windows":
        try:
            ...
            return "Recycle bin emptied." if result == 0 else "Recycle bin is already empty or an error occurred."
        except Exception as e:
            return f"Failed to empty recycle bin: {e}"
    elif _OS == "Darwin":
        import os
        subprocess.run(["rm", "-rf", os.path.expanduser("~/.Trash/*")], capture_output=True)
    ...
    try:
        ...
        for proc in psutil.process_iter(['name']):
            try:
                ...
                pass
        return f"Killed {killed} processes matching '{app_name}'." if killed > 0 else f"No processes found matching '{app_name}'."
    except Exception as e:
...
def enable_bluetooth():
    if _OS == "Windows":
        try:
            subprocess.run(["powershell", "-Command", "If ((Get-Service bthserv).Status -eq 'Stopped') { Start-Service bthserv }"], capture_output=True)
            return "Bluetooth enabled."
        except Exception as e:
            return f"Failed to enable Bluetooth: {e}"
    elif _OS == "Darwin":
        subprocess.run(["blueutil", "-p", "1"], capture_output=True)
        return "Bluetooth enabled."
    else:
...
def disable_bluetooth():
    if _OS == "Windows":
        try:
            subprocess.run(["powershell", "-Command", "If ((Get-Service bthserv).Status -eq 'Running') { Stop-Service bthserv -Force }"], capture_output=True)
...
def list_open_windows():
    if _OS == "Windows":
        try:
            ...
            return f"Failed to get open windows (ensure pygetwindow is installed): {e}"
    elif _OS == "Darwin":
        script = 'tell application "System Events" to get name of every application process whose visible is true'
        result = subprocess.run(["osascript", "-e", script], capture_output=True, text=True)
        return f"Open Apps: {result.stdout.strip()}"
    else:
        result = subprocess.run(["wmctrl", "-l"], capture_output=True, text=True)
        return f"Open Windows: {result.stdout.strip()}"

def read_active_text():
    if not _PYPERCLIP or not _PYAUTOGUI:
        return "Requires pyperclip and pyautogui."
    try:
        # Copy current selection if any, else select all and copy
        pyautogui.hotkey("ctrl", "a" if _OS != "Darwin" else "command", "a")
        time.sleep(0.1)
        pyautogui.hotkey("ctrl", "c" if _OS != "Darwin" else "command", "c")
        time.sleep(0.1)
        pyautogui.press("right") # Deselect
        text = pyperclip.paste()
        return f"Active Text ({len(text)} chars):\n{text[:1000]}..." if text else "No text found in active window."
    except Exception as e:
...
ACTION_MAP: dict[str, callable] = {
    ...
    "read_active_text":    read_active_text,
}

_DANGEROUS_ACTIONS = {"restart", "shutdown", "empty_recycle_bin", "empty_trash"}
...
def computer_settings(
    ...
    if action == "kill_process" or action == "kill":
        app_name = str(value or params.get("app_name", "")).strip()
        if not app_name:
            app_name = description.replace("kill", "").strip()
    ...
    try:
        result = func()
        # if func returns a string, use it
