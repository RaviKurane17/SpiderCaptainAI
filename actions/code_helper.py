...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"captain_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"captain_debug_{int(time.time())}.png"
...
def _open_in_vscode(path: Path):
    """Open a specific file in VS Code. Tries multiple methods on Windows."""
    import time as _t
    file_str = str(path.resolve())
    _t.sleep(0.3)  # Let the file system flush
    ...
    try:
        ...
        if code_bin:
            subprocess.Popen(
                [code_bin, "--reuse-window", file_str],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
            print(f"[Code] ✅ Opened in VS Code: {file_str}")
            return
    except Exception as e:
        print(f"[Code] ⚠️ VS Code via PATH failed: {e}")
    ...
    if platform.system() == "Windows":
        ...
        for exe in candidates:
            if os.path.isfile(exe):
                try:
                    subprocess.Popen(
                        [exe, "--reuse-window", file_str],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL
                    )
                    print(f"[Code] ✅ Opened in VS Code (fallback): {file_str}")
                    return
                except Exception:
                    continue
        ...
        try:
            subprocess.Popen(
                f'code --reuse-window "{file_str}"',
                shell=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
            print(f"[Code] ✅ Opened in VS Code (shell): {file_str}")
        except Exception as e:
            print(f"[Code] ❌ Could not open VS Code: {e}")
...
Updated code:"""
    ...
    _open_in_vscode(Path(file_path))
    return (
        f"File edited. {status}\n"
        f"The file has been opened in VS Code automatically.\n\n"
        f"Preview:\n{_preview(edited)}"
    )
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    if platform.system() == "Windows":
        ...
        if ext == ".py":
            ...
            if " " in py_exe: py_exe = f'"{py_exe}"'
            cmd_str = f'{py_exe} "{p.name}"'
        elif ext == ".js":
            cmd_str = f'node "{p.name}"'
        elif ext == ".java":
            cmd_str = f'javac "{p.name}" && java "{p.stem}"'
        elif ext in (".c", ".cpp"):
            compiler = "gcc" if ext == ".c" else "g++"
            cmd_str = f'{compiler} "{p.name}" -o "{p.stem}.exe" && "{p.stem}.exe"'
        elif ext == ".bat":
            cmd_str = f'"{p.name}"'
        elif ext == ".ps1":
            cmd_str = f'powershell -ExecutionPolicy Bypass -File "{p.name}"'
        ...
        if cmd_str:
            ...
            # Use start to open a new interactive command window
            full_cmd = f'start cmd.exe /c "{cmd_str} & echo. & pause"'
            try:
                subprocess.Popen(full_cmd, shell=True, cwd=str(p.parent))
                return f"Program launched in a new command window. You can interact with it there."
            except Exception as e:
                return f"Could not launch command window: {e}"
                
    # Fallback for non-Windows or unsupported extensions
